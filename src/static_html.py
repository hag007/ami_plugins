...
def generate_report_from_template(cy, module_index, go_report, output_folder):
    ...
    report_file_name=format_script(os.path.join(os.path.dirname(os.path.abspath(__file__)),'../data', "graph.html"), NUM_OF_GENES=len([x for x in cy if not "source" in x["data"]]), GO_REPORT=go_report.loc[:,['index', 'GO id','GO name','GO root','pval', 'qval']].to_dict('records'), GRAPH=json.dumps(cy))
